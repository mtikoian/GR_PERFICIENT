...
/****** Object:  StoredProcedure [dbo].[stp_R_BudgetJobCodeDetail]    Script Date: 12/02/2011 12:04:28 ******/
...
BEGIN
	...
	/* ================================================================================================================
		...
		NOTE: We do not specify parameter defaults. We assume that all parameters have been
		saved correctly and completely in GRP, and we want the stored procedure to break if 
		an expected parameter is not passed through. This prevents unexpected behaviour.
	=================================================================================================================*/
	...
	/* ================================================================================================================
		...
		-- Budget Originator data is only 'Unallocated', 'UNKNOWN'
	...
	SELECT
		...
		ProfitabilityActual.FunctionalDepartmentCode AS 'FunctionalDepartmentCode',
	...
	FROM 
		ProfitabilityActual 
			...
			INNER JOIN GlCategorizationHierarchy GlHierarchy ON GlHierarchy.GLCategorizationHierarchyKey = 
				CASE
	...
	WHERE 
		...
		GlHierarchy.InflowOutflow IN (
			'Outflow', 
			'UNKNOWN'
		) AND
		GlHierarchy.GLMinorCategoryName <> 'Architects & Engineering'
	...
	SELECT
		...
		#BudgetJobCode.EntryDate AS 'EntryDate',
		#BudgetJobCode.[User] AS 'User',
		#BudgetJobCode.[Description] AS 'Description',
		#BudgetJobCode.AdditionalDescription AS 'AdditionalDescription',
		#BudgetJobCode.SourceName AS 'SourceName',
		#BudgetJobCode.PropertyFundCode AS 'PropertyFundCode',
		CASE 
			WHEN (SUBSTRING(#BudgetJobCode.SourceName, CHARINDEX(' ', #BudgetJobCode.SourceName) +1, 8) = 'Property') THEN RTRIM(#BudgetJobCode.OriginatingRegionCode) + LTRIM(#BudgetJobCode.FunctionalDepartmentCode) 
			ELSE #BudgetJobCode.OriginatingRegionCode 
		END AS 'OriginatingRegionCode',
