...
---- Scenario 1: New Record in SOURCE
	...
	BEGIN TRAN
		...
		SELECT * FROM dbo.AllocationRegion WHERE SnapshotId = 0
			...
			EXEC csp_IU_SCDAllocationRegion @DataPriorToDate = '2011-12-31'
...
---- Scenario 2.1: Record updated in SOURCE - field updated not used by DIMENSION
	...
	BEGIN TRAN
		...
		SELECT * FROM dbo.AllocationRegion WHERE SnapshotId = 0
			...
			UPDATE GrReportingStaging.Gdm.GlobalRegion SET DefaultCorporateSourceCode = 'US', UpdatedDate = GETDATE() WHERE Code = 'GER'
			EXEC csp_IU_SCDAllocationRegion  @DataPriorToDate = '2011-12-31'
...
---- Scenario 2.2: Record updated in SOURCE - field updates used by DIMENSION
	...
	BEGIN TRAN
		...
		SELECT * FROM dbo.AllocationRegion WHERE SnapshotId = 0
			...
			UPDATE GrReportingStaging.Gdm.GlobalRegion SET Name = Name + '(U)', UpdatedDate = GETDATE() WHERE Code = 'GER'
			EXEC csp_IU_SCDAllocationRegion  @DataPriorToDate = '2011-12-31'
...
---- Scenario 3.1: Record deactivated in SOURCE
	...
	BEGIN TRAN
		...
		SELECT * FROM dbo.AllocationRegion WHERE SnapshotId = 0
			...
			UPDATE GrReportingStaging.Gdm.GlobalRegion SET IsActive = 0, UpdatedDate = GETDATE() WHERE Code = 'IND'
			EXEC csp_IU_SCDAllocationRegion  @DataPriorToDate = '2011-12-31'
...
---- Scenario 3.2: Record hard deleted in SOURCE
	...
	BEGIN TRAN
		...
		SELECT * FROM dbo.AllocationRegion WHERE SnapshotId = 0
			...
			DELETE FROM GrReportingStaging.Gdm.GlobalRegion WHERE Code = 'FLR'
			EXEC csp_IU_SCDAllocationRegion  @DataPriorToDate = '2011-12-31'
