...
/****** Object:  StoredProcedure [dbo].[stp_IU_LoadGrTimeAllocationBudget]    Script Date: 07/09/2012 02:02:38 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[stp_IU_LoadGrTimeAllocationBudget]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[stp_IU_LoadGrTimeAllocationBudget]
GO
...
Description
	...
	TimeAllocationBudget table in the data warehouse (GrReporting.dbo.TimeAllocationBudget). 
	...
	8.	Map table to the #TimeAllocationBudget table with the same structure as the GrReporting.dbo.TimeAllocationBudget table
	--9.	Insert records with unknowns into the ProfitabilityBudgetUnknowns table.
	10. Insert budget records into the GrReporting.dbo.TimeAllocationBudget table
...
CREATE PROCEDURE [dbo].[stp_IU_LoadGrTimeAllocationBudget]	
...
PRINT '####'
PRINT 'stp_IU_LoadGrTimeAllocationBudget'
PRINT '####'
...
BEGIN
	...
	IF ((SELECT ConfiguredValue FROM GrReportingStaging.dbo.SSISConfigurations WHERE ConfigurationFilter = 'CanImportTimeAllocationBudget') = 0)
	BEGIN
		PRINT ('Import Tapas Time Allocation Budget not scheduled in SSISConfigurations')
		RETURN
	END
	...
	BEGIN
		PRINT ('*******************************************************')
		PRINT ('	stp_IU_LoadGrTimeAllocationBudget is quitting because there are no TAPAS budgets set to be imported.')
		PRINT ('*******************************************************')
		RETURN
	END
...
BEGIN
	...
	*/
	...
	Group by   Allocation.BudgetEmployeePayrollAllocationId,Allocation.SnapshotId


	PRINT 'Completed inserting records into #BudgetEmployeePayrollAllocationDetail: '+CONVERT(char(10),@@rowcount)
	PRINT (CONVERT(VARCHAR(27), DATEDIFF(millisecond, @StartTime, GETDATE()), 121) + ' ms' + CHAR(13))

	CREATE UNIQUE CLUSTERED INDEX IX_BudgetEmployeePayrollAllocationDetailId ON #BudgetEmployeePayrollAllocationDetail (BudgetEmployeePayrollAllocationId)
...
BEGIN
	...
	*/
	...
	SELECT 
		...
		Allocation.BudgetEmployeePayrollAllocationId,
		'TGB:BudgetId=' + CONVERT(varchar,Budget.BudgetId) + '&BudgetProjectId=' + CONVERT(varchar,ISNULL(BudgetProject.BudgetProjectId,-1)) + '&BudgetEmployeeId=' + CONVERT(varchar,ISNULL(BudgetEmployee.BudgetEmployeeId,-1)) + '&BudgetEmployeePayrollAllocationId=' + CONVERT(varchar,Allocation.BudgetEmployeePayrollAllocationId)  AS ReferenceCode,
	...
	FROM
		...
		LEFT JOIN #BudgetProject BudgetProject ON 
		...
		LEFT JOIN #BudgetProjectGroup BudgetProjectGroup ON 
			Allocation.BudgetProjectGroupId = BudgetProjectGroup.BudgetProjectGroupId AND
...
BEGIN
	...
	CREATE TABLE #TimeAllocationBudget
	(
		ImportBatchId INT NOT NULL,
		BudgetReforecastTypeKey INT NOT NULL,
		ReforecastKey INT NOT NULL,
	...
	INSERT INTO #TimeAllocationBudget 
	(
		ImportBatchId, 
		BudgetReforecastTypeKey,
		ReforecastKey,
	...
	SELECT
		...
		GrReporting.dbo.GetLocalNonLocalStr(GrOr.RegionName,GrOr.SubRegionName,GrAr.RegionName,GrAr.SubRegionName)  Local_NonLocal,
	...
	FROM
		...
		LEFT JOIN GrReportingStaging.GACS.Staff BudgetOwner  ON
			pbm.BudgetOwnerStaffId=BudgetOwner.StaffId 
...
--where pbm.BudgetEmployeePayrollAllocationId=22851789	
    ...
    --debug
    --select  * into debug._TimeAllocationBudget from #TimeAllocationBudget

--return 0
	PRINT 'Completed inserting records into #TimeAllocationBudget: '+CONVERT(char(10),@@rowcount)
	PRINT (CONVERT(VARCHAR(27), DATEDIFF(millisecond, @StartTime, GETDATE()), 121) + ' ms' + CHAR(13))
	...
	PRINT 'Completed creating indexes on #TimeAllocationBudget'
	PRINT (CONVERT(VARCHAR(27), DATEDIFF(millisecond, @StartTime, GETDATE()), 121) + ' ms' + CHAR(13))
...
BEGIN
	...
	(
		MERGE 
			GrReporting.dbo.TimeAllocationBudget FACT
		USING
			#TimeAllocationBudget AS SRC ON
...
/* ================================================================================================================================================
	11. Mark budgets as being successfully processed into the warehouse
	should be done in stp_IU_LoadGrProfitabiltyPayrollOriginalBudget, so this stp should be run before that one.
   ============================================================================================================================================= */
/*
BEGIN
