...
/****** Object:  StoredProcedure [dbo].[csp_IU_SCDPropertyFundFTE]    Script Date: 08/14/2012 02:39:52 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[csp_IU_SCDPropertyFundFTE]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[csp_IU_SCDPropertyFundFTE]
GO
...
CREATE PROCEDURE [dbo].[csp_IU_SCDPropertyFundFTE]
			...
			CASE
				WHEN -- If this record (identified by its primary key) doesn't exist in the dimension (regardless of its StartDate and EndDate)
					0 = (	SELECT
								COUNT(*)
							FROM
								GrReporting.dbo.PropertyFundFTE
							WHERE
								SnapshotId = 0 AND
			...
			/*
				...
				To achieve this:
					...
					First, try and find the maximum EndDate in the dimension for the source record that is to be inserted.
						...
						THEN/[DATEADD(ms, 10, (SELECT MAX(EndDate) FROM GrReporting.dbo.PropertyFundFTE WHERE PropertyFundIdId = SRC.PropertyFundIdId))]
...
INSERT INTO dbo.PropertyFundFTE
(
	PropertyFundId,
	PropertyFundName,
	PropertyFundType,
...
SELECT
	...
	DATEADD(MS, 10, (SELECT MAX(EndDate) FROM GrReporting.dbo.PropertyFundFTE DIM WHERE PropertyFund.PropertyFundId = DIM.PropertyFundId AND DIM.SnapshotId = PropertyFund.SnapshotId)) AS StartDate,
