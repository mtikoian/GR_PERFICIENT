...
SELECT 
	...
	SUM(
            er.Rate *
            CASE WHEN (c.CalendarPeriod <= ' + STR(@ReportExpensePeriod,6,0) + ' AND c.CalendarPeriod < ' + STR(@PreviousReforecastEffectivePeriod,6,0) + 
                  /*Hack to pull non-payroll actuals from the grinder for Q1, 201003... yes 201003 Q1. (Ask MikeC)*/ 
                  /*Exclude actual... actuals here so we don't double count on the grinder actuals*/
                  ' AND (gac.ExpenseType <> ''Non-Payroll'' OR ' + STR(@PreviousReforecastEffectivePeriod,6,0) + ' <> 201003)) THEN 
                  pa.LocalActual
            ELSE
                  0
            END
...
SELECT 
	...
	(
		er.Rate *
		CASE WHEN (c.CalendarPeriod <= '+STR(@ReportExpensePeriod,10,0)+' AND c.CalendarPeriod < ' + STR(@PreviousReforecastEffectivePeriod,6,0) + 
			/*Hack to pull non-payroll actuals from the grinder for Q1, 201003... yes 201003 Q1. (Ask MikeC)*/ 
			/*Exclude actual... actuals here so we don't double count on the grinder actuals*/
			' AND (gac.ExpenseType <> ''Non-Payroll'' OR ' + STR(@PreviousReforecastEffectivePeriod,6,0) + ' <> 201003)) THEN 
			pa.LocalActual
