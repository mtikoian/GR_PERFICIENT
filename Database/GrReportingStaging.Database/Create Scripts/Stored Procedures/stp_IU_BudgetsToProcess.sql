...
BEGIN
	...
	*/
	...
	SELECT
		...
		S.ManualUpdatedDate
	FROM
		GDM.dbo.BudgetReportGroupPeriod BRGP
		...
		(
			...
			FROM
				TAPASUS_Budgeting.Budget.Budget BudgetTAPAS

				INNER JOIN TAPASUS_Budgeting.[Admin].BudgetReportGroupDetail BRGD ON
					BudgetTAPAS.BudgetId = BRGD.BudgetId

				INNER JOIN TAPASUS_Budgeting.[Admin].BudgetReportGroup BRG ON
					BRGD.BudgetReportGroupId = BRG.BudgetReportGroupId

				INNER JOIN GDM.dbo.BudgetReportGroupPeriod BRGP ON
		...
		INNER JOIN GDM.dbo.[Snapshot] S ON
