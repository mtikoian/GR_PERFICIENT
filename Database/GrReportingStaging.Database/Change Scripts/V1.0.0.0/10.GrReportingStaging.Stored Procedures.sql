...
/****** Object:  StoredProcedure [dbo].[stp_IU_LoadGrProfitabiltyPayrollOriginalBudget]    Script Date: 12/29/2009 12:07:48 ******/
...
/****** Object:  StoredProcedure [dbo].[stp_IU_LoadGrProfitabiltyGeneralLedger]    Script Date: 12/29/2009 12:07:48 ******/
...
/****** Object:  StoredProcedure [dbo].[stp_IU_LoadGrProfitabiltyCorporateOriginalBudget]    Script Date: 12/29/2009 12:07:48 ******/
...
/****** Object:  StoredProcedure [dbo].[stp_IU_LoadGrProfitabiltyOverhead]    Script Date: 12/29/2009 12:07:48 ******/
...
/****** Object:  StoredProcedure [dbo].[stp_IU_LoadExchangeRates]    Script Date: 12/29/2009 12:07:48 ******/
...
/****** Object:  StoredProcedure [dbo].[stp_I_ImportBatch]    Script Date: 12/29/2009 12:07:48 ******/
...
/****** Object:  StoredProcedure [dbo].[stp_U_ImportBatch]    Script Date: 12/29/2009 12:07:48 ******/
...
/****** Object:  StoredProcedure [dbo].[stp_IU_TapasGlobalBudgeting_ProfitabilityBudgetEmployeePayrollAllocation]    Script Date: 12/29/2009 12:07:48 ******/
...
FROM
	...
	LEFT OUTER JOIN 
		#PropertyFund OverheadPropertyFund ON
				OverheadProject.PropertyFundId = OverheadPropertyFund.PropertyFundId
...
SELECT 
	ProExists.ProfitabilityBudgetKey,
	ISNULL(GlAc.GlAccountCategoryKey, ISNULL(GlAcUnknown.GlAccountCategoryKey, -1)) GlAccountCategoryKey
INTO 
	#ProfitabilityBudgetGlAccountCategoryBridge
FROM 
		(SELECT 
		...
		LEFT OUTER JOIN #GlobalGlAccountCategoryHierarchy GlAcH ON 
			GlAcH.GlobalGlAccountId = GrGa.GlobalGlAccountId AND 
		...
		LEFT OUTER JOIN GrReporting.dbo.GlAccountCategory GlAc ON 
			GlAc.GlobalGlAccountCategoryCode = LTRIM(STR(GlAcHg.GlobalGlAccountCategoryHierarchyGroupId,10,0))+':'+
												LTRIM(STR(MaGlAc.MajorGlAccountCategoryId,10,0))+':'+
												LTRIM(STR(MiGlAc.MinorGlAccountCategoryId,10,0)) AND
...
Select 
	...
	CASE WHEN (P1.AllocateOverheadsProjectId IS NULL OR P1.AllocateOverheadsProjectId = 0) THEN 
		IsNull(CASE WHEN (P1.CorporateDepartmentCode = '@' OR P1.CorporateDepartmentCode IS NULL) THEN 
					ProjectPropertyFund.PropertyFundId
				ELSE 
					DepartmentPropertyFund.PropertyFundId 
				END, -1) 
...
From 
		...
		LEFT OUTER JOIN #PropertyFund ProjectPropertyFund ON
			P1.PropertyFundId = ProjectPropertyFund.PropertyFundId

		LEFT OUTER JOIN #PropertyFund OverheadPropertyFund ON
			P2.PropertyFundId = OverheadPropertyFund.PropertyFundId
...
-- EmployeePayrollAllocationTotals
...
-- ProfitabilityBudgetEmployeePayrollAllocation
...
FROM
	...
	LEFT OUTER JOIN TapasGlobalBudgeting.BudgetEmployeeFunctionalDepartment EFD ON
	...
	LEFT OUTER JOIN Gdm.PropertyFundMapping PFM ON
		BudgetProject.PropertyFundId = PFM.PropertyFundId
