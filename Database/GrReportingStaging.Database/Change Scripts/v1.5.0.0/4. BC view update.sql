...
/****** Object:  View [dbo].[GlobalReportingCorporateBudget]    Script Date: 11/22/2010 13:42:15 ******/
...
FROM 
	...
	INNER JOIN BC_TS_GRINDER_Q3.dbo.NonPayrollExpense NPE ON  
		NPED.NonPayrollExpenseId = NPE.NonPayrollExpenseId	

	INNER JOIN BC_TS_GRINDER_Q3.dbo.NonPayrollCycle NPC ON  
		NPED.NonPayrollCycleUID = NPC.NonPayrollCycleUID
/* -- Doesn't seem like they are using versioning for non-payroll, yet.
	INNER JOIN (
		SELECT NonPayrollCycleUID, max(Version) as Version
		...
		NPED.Version = NPEDV.Version
*/
	INNER JOIN BC_TS_GRINDER_Q3.dbo.ProjectGroup PG ON  
		NPE.ProjectGroupId = PG.ProjectGroupId

	INNER JOIN BC_TS_GRINDER_Q3.dbo.ProjectGroupAllocation PGA ON  
		PG.ProjectGroupId = PGA.ProjectGroupId

	INNER JOIN BC_TS_GRINDER_Q3.dbo.Project Proj ON  
		PGA.ProjectId = Proj.ProjectId

    INNER JOIN BC_TS_GRINDER_Q3.dbo.GLType GLT ON 
        NPE.GLTypeId = GLT.GLTypeId
	
    INNER JOIN BC_TS_GRINDER_Q3.dbo.ActivityType AT ON  
        Proj.ActivityTypeId = AT.ActivityTypeId

	LEFT OUTER JOIN BC_TS_GRINDER_Q3.dbo.GlobalAccountMappingLookup GLAC ON  
		NPE.GLTypeId = GLAC.GLTypeId AND
		Proj.ActivityTypeId = GLAC.ActivityTypeId

	LEFT OUTER JOIN BC_TS_GRINDER_Q3.dbo.GlobalAccountMapping GAM ON  
		GLAC.GlobalAccountMappingId = GAM.GlobalAccountMappingId

    LEFT OUTER JOIN BC_TS_GRINDER_Q3.dbo.FunctionalDepartment FD ON  
        NPE.OriginatingFunctionalDepartmentId = FD.FunctionalDepartmentId

    LEFT OUTER JOIN BC_TS_GRINDER_Q3.dbo.EmployeeSubRegion ESR ON  
        NPE.OriginatingEmployeeSubRegionId = ESR.EmployeeSubRegionId

    LEFT OUTER JOIN BC_TS_GRINDER_Q3.dbo.EmployeeRegion ER ON  
        ESR.EmployeeRegionId = ER.EmployeeRegionId

    INNER JOIN BC_TS_GRINDER_Q3.dbo.Currency C ON   
        NPE.CurrencyId = C.CurrencyId
        
    LEFT OUTER JOIN BC_TS_GRINDER_Q3.dbo.JobCode JC ON  
        NPE.JobCodeId = JC.JobCodeId

    LEFT OUTER JOIN BC_TS_GRINDER_Q3.dbo.SubRegion SR ON  
        Proj.SubRegionId = SR.SubRegionId

	LEFT OUTER JOIN BC_TS_GRINDER_Q3.dbo.Region R ON  
		Proj.RegionId = R.RegionId		
WHERE
	NPC.IsAvailable = 1 AND NPC.AvailableDate IS NOT NULL AND NPC.BudgetPeriodCode = 'Q3'-- AND
	--Change Control 1 :: GC 
	--NPE.Overhead = 0
...
FROM 
	BC_TS_GRINDER_Q3.dbo.FeeBudgetDetail   --

    --We need to unpivot to turn the local amounts into rows rather than columns
	UNPIVOT (Amount FOR Period IN (January, February, March, April, May, June, July, August, September, October, November, December)) AS FBD

	INNER JOIN BC_TS_GRINDER_Q3.dbo.FeeDetail FeeD ON  --
		FBD.FeeDetailUID = FeeD.FeeDetailUID	
		
	LEFT OUTER JOIN BC_TS_GRINDER_Q3.dbo.PropFundMRIDeptCodeMapping PFDCM ON
		FeeD.PropFundMRIDeptCodeMappingId = PFDCM.PropFundMRIDeptCodeMappingId

	INNER JOIN BC_TS_GRINDER_Q3.dbo.FeeBudgetCycle FBC ON  --BC_TS_GRINDER_Q3
		FBD.FeeBudgetCycleUID = FBC.FeeBudgetCycleUID
/* -- Doesn't seem like they are using versioning for fees, yet. 
	INNER JOIN (
		SELECT FeeBudgetCycleUID, max(Version) as Version
		...
		FBD.Version = FBDV.Version
*/
    LEFT OUTER JOIN BC_TS_GRINDER_Q3.dbo.GlobalAccountMappingLookupFee GLAC ON  --
        IsNull(FeeD.FeeTypeId,0) = GLAC.FeeTypeUID AND
        IsNull(FeeD.FeeCategoryId,0) = GLAC.FeeCategoryUID
...
WHERE
	FBC.IsAvailable = 1 AND 
	FBC.AvailableDate IS NOT NULL AND FBC.BudgetPeriodCode = 'Q3' AND
	FBC.NonPayrollCycleUID IS NOT NULL	
